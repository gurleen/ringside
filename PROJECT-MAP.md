...
| `lib/share-image.ts`                       | `getWrestlerHeadshotDataUrls` server fn (+ query options): fetches up to 4 SDH headshots server-side (ids resolved via crosswalk, never raw URLs) and returns base64 data URLs so the share-card `html-to-image` export never fetches cross-origin.                                                                                                                                                                                                                                                                                                                                 |
...
| `components/prediction-side-picker.tsx`    | Per-match winner picker / locked status for event cards (`useMutation` → upsert/delete).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `components/admin-match-result-menu.tsx`   | Admin-only 3-dot dropdown in the match-card header: “Set Result…” opens a `Dialog` to pick the winning side (Save → `setMatchResult` by `side.id`); “Clear result…” confirms via `AlertDialog` → `clearMatchResult`.                                                                                                                                                                                                                                                                                                                                                                 |
...
| `components/ui/*`                          | shadcn components (card, button, input, textarea, badge, table, skeleton, separator, switch, label, select, collapsible, avatar, tabs, tooltip, dropdown-menu, sheet, alert-dialog, dialog).                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **Wrestler detail** (`/wrestlers/$wrestlerId?tab=&page=`): `getWrestler`
  ...
  **Profile** shows the Attributes card, then previous/next bout
  ...
  `listRivalryMatches` focused on the current wrestler (Winner column with the
  same Spoilers blur toggle as the rivalry page); an **Include matches with
...
- **Rivalries** (`/rivalries/$rivalryKey?page=`): all-time matches whose
  ...
  additional participants. Table columns are Date, Event, and Winner; a
  **Spoilers** switch (off by default) un-blurs the winner
  name(s) or non-decisive finish label (`finish_note` such as “No Contest” /
  “Time Limit Draw”, else “No Contest” when `result` is `no_decision`/`unknown`).
  Unresolved rows with no finish info still show "—". Header links each wrestler
...
- **Match predictions** (`/events/$eventId`, `/predictions`, `/leaderboard`):
  ...
  Leaderboard is all-time only. “My Predictions” is in the account menu;
  Leaderboard is in the primary nav.
