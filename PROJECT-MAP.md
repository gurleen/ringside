...
| `routes/index.tsx`                         | Dashboard: curated top champions grid (AEW / Japan / WWE Raw / WWE SmackDown) + recent events.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Auth** (`/login`, `/signup`, `/auth/confirm`): email/password via Supabase
  ...
  `code` for a session.
- **Dashboard** (`/`): curated top champions (`getWorldChampions` — 4×2 grid:
  AEW, Japan (IWGP Heavyweight + World of Stardom), WWE Raw, WWE SmackDown;
  men left / women right per row; SDH wrestler portraits + belt art) and the 8
  most recent past events (`getRecentEvents`). Champions are computed live (not
  from `dashboard_cache.world_champions`). Recent events still read from
  `dashboard_cache` (pg_cron every 10 min) with live fallback.
